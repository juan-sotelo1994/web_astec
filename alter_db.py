...
def inicializar_base_datos():
    """
    Función para recrear y poblar la estructura de la base de datos `astec`.
    Se debe llamar solo para inicializar el sistema de cero.
    """
    try:
        ...
        cursor = mydb.cursor()
        cursor.execute("DROP DATABASE IF EXISTS astec")
        cursor.execute("CREATE DATABASE astec")
        cursor.execute("USE astec")
    ...
    "CREATE INDEX idx_token_email ON password_reset_tokens(email)"
        ...
        for s in statements:
            try:
                ...
                print("ERROR", e, "ON", s[:50])
        
        mydb.commit()
        print("Base de datos ASTEC inicializada correctamente.")
    except Exception as e:
        print("Error de conexión al inicializar BD:", e)
    finally:
        if 'cursor' in locals():
            cursor.close()
