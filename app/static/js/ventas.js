...
function addLinea() {
    ...
    changeLineType(row.querySelector('.line-tipo'));
}
...
function toggleForm() {
    ...
    if (formContainer.style.display === "none") {
        ...
        btnToggle.innerHTML = '<i class="fa-solid fa-plus"></i> Nueva Venta';
        btnToggle.style.backgroundColor = '#005b9f';
        cancelEdit();
    }
}

function editVenta(id, cliente_id, vendedor_id, cotizacion_id, metodo_pago, estado, total) {
    document.getElementById('id_edit').value = id;
    document.getElementById('cliente_id').value = cliente_id;
    document.getElementById('vendedor_id').value = vendedor_id;
    document.getElementById('cotizacion_id').value = cotizacion_id;
    document.getElementById('metodo_pago').value = metodo_pago;
    document.getElementById('estado_venta').value = estado;
    document.getElementById('total').value = parseFloat(total).toFixed(2);

    document.getElementById('formTitle').textContent = 'Editar Venta (Parcial)';
    document.getElementById('btnSubmitText').textContent = 'Actualizar Venta';
    document.getElementById('btnCancelEdit').style.display = 'block';
    document.getElementById('lineas-container').innerHTML = '<p style="color:#d97706; font-size: 13px;"><i class="fa-solid fa-warning"></i> La edición de líneas no está activa en esta vista. Si guardas, los detalles serán ignorados pero la venta maestro se actualizará.</p>';
...
function cancelEdit() {
    ...
    document.getElementById('btnSubmitText').textContent = 'Procesar Venta';
